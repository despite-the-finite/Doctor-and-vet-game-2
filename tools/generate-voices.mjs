...
import { textToSpeech, voiceExists, MODELS, DEFAULT_MODEL, DEFAULT_FORMAT } from './lib/elevenlabs.mjs';
...
if (opts.check || registryProblems.length) {
  ...
  if (placeholders.length) {
    console.log(`\n  ${placeholders.length} character(s) are still using premade stand-in voices.`);
...
for (const entry of collected) {
  ...
  // `old.missing` matters as much as the fingerprint. A --limit run, or a
  // clip that failed, writes the NEW fingerprint for a line it never actually
  // generated; without this test the next run sees a matching fingerprint and
  // a file on disk (the STALE one from before the text changed), calls the
  // line up to date and never regenerates it — so the game ships old audio
  // for a line that has since been rewritten.
  const hadAudio = old && old.missing !== true;
  const unchanged = hadAudio && old.fingerprint === fingerprint && old.file === file && onDisk;
...
/* Every distinct voice, checked once, before a single clip is paid for. */
const voiceIds = [...new Set(limited.map((p) => p.voice.voiceId))];
const badVoices = [];
await pool(voiceIds, 4, async (voiceId) => {
  const result = await voiceExists({ apiKey, voiceId });
  if (!result.ok) badVoices.push({ voiceId, reason: result.reason });
});
if (badVoices.length) {
  console.error(`\n✖ ${badVoices.length} voice id(s) are not usable on this account:\n`);
  for (const bad of badVoices) {
    const who = Object.keys(VOICES).filter((id) => VOICES[id].voiceId === bad.voiceId);
    console.error(`   ${bad.voiceId}  (${who.join(', ') || 'unused'}) — ${bad.reason}`);
  }
  console.error('\n  Run "npm run voices:list" to see the voices you do have, and paste');
  console.error('  the ids into src/dialogue/voices.js. Nothing has been generated.\n');
  process.exit(1);
}
...
if (stillPlaceholder.length) {
  console.log(`\n  ⚠ ${stillPlaceholder.length} character(s) still use premade stand-in voices:`);
...
function writeManifest() {
  ...
  // The other half of the same guard: a record with no audio must not carry
  // the fingerprint of the audio it was going to have. Blanking it means the
  // next run always re-plans the line, whatever is lying around on disk.
  records.forEach((r) => { if (r.missing) r.fingerprint = null; });
