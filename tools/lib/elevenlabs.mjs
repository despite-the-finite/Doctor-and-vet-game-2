...
export async function textToSpeech({
  ...
  // `apply_text_normalization` is only accepted by some models — v3 and the
  // turbo/flash pair reject or ignore it, and 'auto' is what they do anyway.
  // Sending it where it is not supported turns every request into a 4xx.
  if (spec.textNormalization) body.apply_text_normalization = 'auto';
...
/**
 * Does this voice id exist on this account?
 *
 * A mistyped or retired id does not fail loudly — it fails once per line, so
 * a run can burn through two thousand requests before anyone notices the
 * narrator is missing. Checking each distinct id once, up front, turns that
 * into a single sentence before anything is spent.
 */
export async function voiceExists({ apiKey, voiceId }) {
  try {
    const res = await fetch(`${API_BASE}/v1/voices/${encodeURIComponent(voiceId)}`, {
      headers: { 'xi-api-key': apiKey },
    });
    if (res.ok) return { ok: true, name: (await res.json())?.name || null };
    if (res.status === 404 || res.status === 400) return { ok: false, reason: 'not on this account' };
