...
Every character ships with a **premade placeholder** so the pipeline runs
before you have decided anything, and no two characters in the same track
share one — the registry used to put four voices across nineteen parts, which
meant the Doctor narrator and the child patient were the same person, and a
child cannot follow a scene where the question and the answer sound identical.

The placeholders are still a cast picked from a public list. Choosing your own
is the step that turns a working pipeline into a game worth listening to.
...
`voices:check` also verifies every id against your account, and
`generate-voices` runs the same check before it spends anything — a mistyped
or retired id is one sentence up front rather than two thousand failed
requests.
